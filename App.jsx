...
export default function Dashboard() {
  ...
  const MiniTask = ({ t }) => (
    <button onClick={() => toggleTask(t.id)} className="w-full text-left flex items-center gap-2 px-2.5 py-2 rounded-md hover:bg-white">
      ...
      <span className="flex items-center gap-1 shrink-0">
        {t.owner.slice(0, 2).map((o) => <span key={o} className="rounded-full" style={{ width: 7, height: 7, background: (TEAM[o] || TEAM.all).color }} />)}
      ...
      <main className="flex-1 min-w-0">
        ...
        <header className="relative overflow-hidden text-white px-6 py-4" style={{ background: "radial-gradient(120% 160% at 90% -20%, #2a3473 0%, #141a35 50%, #0b0f23 100%)" }}>
          ...
          <div className="relative flex items-center justify-between gap-4 flex-wrap">
            <div className="text-lg font-semibold tracking-tight">AstroBizNet — Mission Control</div>
            <div className="text-[11px] text-indigo-200/90 tabular-nums">{counts.done} of {counts.total} complete · {counts.pct}% · {daysLeft} days left</div>
          </div>
    ...
    return (
      <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <SectionLabel><Bell size={13} className="inline -mt-0.5 mr-1" /> Action centre</SectionLabel>
            <div className="bg-white rounded-xl border border-slate-200 p-1.5 mb-4">
              {[...buckets.overdue, ...buckets.week].length === 0
                ? <div className="text-[12px] text-slate-400 px-2.5 py-3 text-center">Nothing urgent right now 🎉</div>
                : [...buckets.overdue, ...buckets.week].slice(0, 6).map((t) => <MiniTask key={t.id} t={t} />)}
            </div>
            <SectionLabel>Don't get caught out</SectionLabel>
            <div className="bg-white rounded-xl border border-slate-200 p-3.5 text-[12px] text-slate-600 space-y-2">
              <div className="flex gap-2"><Wallet size={14} className="shrink-0 mt-0.5 text-teal-600" /><span><b>Keep every receipt.</b> The finance report needs proof of expenditure.</span></div>
              <div className="flex gap-2"><Clock size={14} className="shrink-0 mt-0.5 text-amber-600" /><span>Final report due <b>within 30 days</b> of the OAD's request.</span></div>
              <div className="flex gap-2"><Flag size={14} className="shrink-0 mt-0.5 text-indigo-500" /><span>Tell the OAD <b>before</b> any major budget or timeline change.</span></div>
            </div>
          </div>
          <div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4">
              {nextM ? <>
                <div className="text-[11px] text-slate-400 mb-0.5">{fmtLong(nextM.date)} · {daysTo(nextM.date) >= 0 ? daysTo(nextM.date) + " days away" : "overdue"}</div>
                <div className="text-sm font-semibold text-slate-800">{nextM.title}</div>
                {nextM.desc && <div className="text-[12px] text-slate-500 mt-1">{nextM.desc}</div>}
              </> : <div className="text-[12px] text-slate-400">No upcoming milestones.</div>}
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-1.5">
              {board.phases.map((p) => { const dc = p.tasks.filter((t) => t.done).length, tc = p.tasks.length, pct = tc ? Math.round(dc/tc*100) : 0; return (
                <button key={p.id} onClick={() => { setTab("tasks"); setOpenPhaseId(p.id); }} className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-md hover:bg-slate-50">
                  <span className="rounded-full shrink-0" style={{ width: 8, height: 8, background: p.accent }} />
                  <span className="flex-1 text-left text-[12px] text-slate-700 truncate">{p.label}</span>
                  <div className="w-12 h-1 rounded-full bg-slate-100 overflow-hidden shrink-0"><div className="h-full rounded-full" style={{ width: pct + "%", background: p.accent }} /></div>
                  <span className="text-[10px] text-slate-400 tabular-nums w-7 text-right">{dc}/{tc}</span>
                </button>
              ); })}
            </div>
          </div>
  ...
  function FilterBar() {
    return (
      <div className="flex items-center gap-2 flex-wrap mb-4">
        ...
        <button onClick={() => setOwnerFilter("all")} className={"text-xs font-medium px-3 py-1.5 rounded-full border " + (ownerFilter === "all" ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-200")}>Everyone</button>
        {TEAM_IDS.map((id) => { const active = ownerFilter === id, m = TEAM[id]; return (
          <button key={id} onClick={() => setOwnerFilter(id)} className="text-xs font-medium px-3 py-1.5 rounded-full border inline-flex items-center gap-1.5" style={active ? { background: m.color, borderColor: m.color, color: "#fff" } : chipInactive}>
            <span className="rounded-full" style={{ width: 7, height: 7, background: active ? "#fff" : m.color }} />{m.name}
          </button>); })}
      </div>
    );
  }
      ...
      <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {board.phases.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => setOpenPhaseId(p.id)} className="text-left bg-white rounded-xl border border-slate-200 hover:border-indigo-400 hover:-translate-y-0.5 transition-all overflow-hidden group">
                <div className="p-3.5 pb-2.5 flex items-start gap-2.5">
                  <span className="rounded-full shrink-0 mt-1" style={{ width: 10, height: 10, background: p.accent }} />
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold text-slate-800 leading-tight">{p.label}</div>
                    <div className="text-[10px] uppercase tracking-wider text-slate-400 mt-0.5">{p.month}</div>
                  </div>
                  <span onClick={(e) => { e.stopPropagation(); openEditPhase(p); }} className="p-1 rounded-md text-slate-300 hover:text-indigo-600 hover:bg-slate-50 opacity-0 group-hover:opacity-100"><Pencil size={13} /></span>
                </div>
                <div className="px-3.5 pb-3">
                  <div className="h-1 rounded-full bg-slate-100 overflow-hidden mb-1.5"><div className="h-full rounded-full" style={{ width: pct + "%", background: p.accent }} /></div>
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-medium text-slate-500">{dc}/{tc} tasks</span>
                    <div className="flex gap-1">
                      {overdue > 0 && <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "#FCEBEB", color: "#A32D2D" }}>{overdue} overdue</span>}
                      {week > 0 && <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "#FAEEDA", color: "#854F0B" }}>{week} this week</span>}
                      {dc === tc && tc > 0 && <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "#EAF3DE", color: "#3B6D11" }}>Done</span>}
                    </div>
                  </div>
                </div>
              </button>
          ...
          <button onClick={openNewPhase} className="rounded-xl border border-dashed border-slate-300 text-slate-400 hover:border-indigo-400 hover:text-indigo-600 hover:bg-white flex items-center justify-center gap-2 text-[13px] min-h-[96px]">
    ...
    return (
      <>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={openNewMilestone} className="text-xs px-2.5 py-1.5 rounded-md border border-slate-200 text-slate-500 hover:text-indigo-600 hover:border-indigo-300 inline-flex items-center gap-1"><Plus size={13} /> Add milestone</button>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          {board.milestones.map((m, i) => {
            const d = m.date ? daysTo(m.date) : null;
            const dotColor = m.state === "done" ? "#10b981" : m.state === "now" ? "#4f46e5" : "transparent";
            const last = i === board.milestones.length - 1;
            return (
              <div key={m.id} className="group flex gap-3 pb-4 relative">
                {!last && <span className="absolute left-[7px] top-4 bottom-0 w-px bg-slate-200" />}
                <span className="w-[15px] h-[15px] rounded-full border-2 shrink-0 mt-0.5 bg-white" style={{ background: m.state === "future" ? "#fff" : dotColor, borderColor: m.state === "future" ? "#cbd5e1" : dotColor }} />
                <div className="flex-1 min-w-0 pb-1">
                  <div className="text-[13px] font-semibold" style={{ color: m.state === "now" ? "#4f46e5" : "#1e293b" }}>{m.title}</div>
                  <div className="text-[11px] text-slate-400 mt-0.5">{fmtLong(m.date)}{d !== null && m.state !== "done" ? (d === 0 ? " · today" : d > 0 ? ` · ${d}d away` : ` · ${Math.abs(d)}d ago`) : ""}</div>
                  {m.desc && <div className="text-[11px] text-slate-500 mt-1">{m.desc}</div>}
                  <div className="flex gap-1.5 mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEditMilestone(m)} className="text-[11px] px-2 py-0.5 rounded border border-slate-200 text-slate-500 hover:text-indigo-600 hover:border-indigo-300 inline-flex items-center gap-1"><Pencil size={11} /> Edit</button>
                    <button onClick={() => removeMilestone(m.id)} className="text-[11px] px-2 py-0.5 rounded border border-slate-200 text-slate-500 hover:text-rose-600 hover:border-rose-300 inline-flex items-center gap-1"><Trash2 size={11} /> Delete</button>
                  </div>
                </div>
              </div>
            );
  ...
  function Team() {
    return (
      <>
        ...
        <div className="bg-white rounded-xl border border-slate-200 px-4 mb-4">
          {TEAM_IDS.map((id, i) => { const m = TEAM[id], s = ownerStats(id); return (
            <div key={id} className={"flex items-center gap-3 py-3 " + (i < TEAM_IDS.length - 1 ? "border-b border-slate-100" : "")}>
              <span className="grid place-items-center rounded-full text-[11px] font-bold text-white shrink-0" style={{ width: 32, height: 32, background: m.color }}>{m.short}</span>
              <div className="flex-1 min-w-0">
                <div className="text-[13px] font-semibold text-slate-800">{m.name}</div>
                <div className="text-[11px] text-slate-400">{m.role}</div>
              </div>
              <div className="shrink-0 text-right">
                <div className="w-20 h-1 rounded-full bg-slate-100 overflow-hidden mb-1"><div className="h-full rounded-full" style={{ width: (s.total ? (s.total - s.open) / s.total * 100 : 0) + "%", background: m.color }} /></div>
                <div className="text-[10px] text-slate-400">{s.open} open</div>
              </div>
            </div>
          ); })}
        </div>
        <SectionLabel>OAD grant contact</SectionLabel>
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-[12px] text-slate-600 space-y-1.5">
          <div className="flex items-center gap-2"><Users size={14} className="text-slate-400" /> Ramasamy Venugopal — OAD Operations & Programs</div>
          <div className="flex items-center gap-2"><Mail size={14} className="text-slate-400" /> <span style={{ color: "#185FA5" }}>rv@astro4dev.org</span></div>
          <div className="flex items-center gap-2"><Globe size={14} className="text-slate-400" /> <span style={{ color: "#185FA5" }}>www.astro4dev.org</span></div>
        </div>
      </>
    );
  }
    ...
    return (
      <>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={openNewEvent} className="text-xs px-2.5 py-1.5 rounded-md border border-slate-200 text-slate-500 hover:text-indigo-600 hover:border-indigo-300 inline-flex items-center gap-1"><Plus size={13} /> Add date</button>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 px-4 mb-4">
          {board.events.map((k, i) => { const past = k.done, d = daysTo(k.date); return (
            <div key={k.id} className={"group flex items-center gap-2.5 py-3 " + (i < board.events.length - 1 ? "border-b border-slate-100" : "")}>
              <Flag size={15} className="shrink-0" style={{ color: past ? "#10b981" : d < 0 ? "#94a3b8" : k.type === "meeting" ? "#6366f1" : "#14b8a6" }} />
              <div className="flex-1 min-w-0">
              ...
              <button onClick={() => openEditEvent(k)} className="shrink-0 p-1 rounded-md text-slate-300 hover:text-slate-600 hover:bg-slate-100 opacity-0 group-hover:opacity-100"><Pencil size={13} /></button>
            </div>
          ); })}
        </div>
        <SectionLabel>Calendar export</SectionLabel>
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between">
          <div className="text-[12px] text-slate-500">Download all deadlines & meetings as .ics (with reminders)</div>
          <button onClick={exportICS} className="text-xs px-3 py-2 rounded-md border border-slate-200 text-slate-700 hover:bg-slate-50 inline-flex items-center gap-1.5"><CalendarPlus size={14} /> Export .ics</button>
        </div>
        <div className="text-center mt-5">
          <button onClick={resetBoard} className="text-[11px] text-slate-400 hover:text-slate-600 inline-flex items-center gap-1"><RotateCcw size={11} /> Reset board to template</button>
        </div>
      </>
    );
  }
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col">
        <div className="flex items-center gap-2.5 px-3.5 py-2.5 border-b border-slate-100">
          <span className="grid place-items-center rounded-full text-[10px] font-bold text-white shrink-0" style={{ width: 26, height: 26, background: m.color }}>{m.short}</span>
          <span className="text-[13px] font-semibold text-slate-800 flex-1">{m.name}</span>
          {saved && <span className="text-[10px] text-emerald-600">Saved ✓</span>}
        </div>
        <textarea ref={ref} defaultValue={board.notes[id] || ""} onInput={() => setSaved(false)} rows={6}
          placeholder="Write anything — blockers, ideas, links, reminders…"
          className="w-full px-3.5 py-3 text-[13px] text-slate-700 resize-y focus:outline-none leading-relaxed" />
        <div className="px-3.5 py-2 border-t border-slate-100 flex justify-end">
          <button onClick={() => { saveNote(id, ref.current.value); setSaved(true); }} className="text-[11px] px-3 py-1.5 rounded-md bg-indigo-600 text-white font-medium hover:bg-indigo-700">Save</button>
        </div>
      </div>
    );
  }
...
function SectionLabel({ children }) {
  return <h3 className="text-[11px] uppercase tracking-[0.14em] font-bold text-slate-500 mb-2.5">{children}</h3>;
}
